...
namespace Apoc3D
{
	namespace Collections
	{
			...
			public:
				...
				bool MoveNext()
				{
					while (m_index<m_dict->m_count)
					{
						if (m_dict->m_entries[m_index].hashCode>=0)
						{
							m_current = &m_dict->m_entries[m_index].key;
							m_currentVal = &m_dict->m_entries[m_index].value;
							m_index++;
							return true;
						}
		...
		public:
			...
			explicit FastMap(int capacity = 8)
			...
			bool Remove(const T& item)
			{
				if (m_buckets)
				{
					int hash = getHashCode(item) & 2147483647;
					int index = hash % m_bucketsLength;
					int lastI = -1;
					for (int i = m_buckets[index]; i >= 0; i = m_entries[i].next)
					{
						if (m_entries[i].hashCode == hash && m_entries[i].key == item)
						{
							if (lastI < 0)
							{
								m_buckets[index] = m_entries[i].next;
		...
		private:
			...
			static int32 getHashCode(const void* ptr) { return static_cast<int32>(reinterpret_cast<uintptr>(ptr)); }
			static int32 getHashCode(int32 val) { return val; }
			static int32 getHashCode(uint32 val) { return static_cast<int32>(val); }
