...
HLSLDeclaration * HLSLTree::FillGlobalDeclaration(Array<HLSLDeclaration*>& results)
{
	HLSLStatement * statement = m_root->statement;
	...
	while (statement != NULL)
	{
		if (statement->nodeType == HLSLNodeType_Declaration)
		{
			results.PushBack((HLSLDeclaration *)statement);
		}
		else if (statement->nodeType == HLSLNodeType_Buffer)
		{
			HLSLBuffer* buffer = (HLSLBuffer*)statement;
			...
			while (field != NULL)
			{
				...
				results.PushBack((HLSLDeclaration *)field);
