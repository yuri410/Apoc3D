...
HLSLBaseType _binaryOpTypeLookup[HLSLBaseType_NumericCount][HLSLBaseType_NumericCount] = 
    {
        { // float
			...
			HLSLBaseType_Float, HLSLBaseType_Float2, HLSLBaseType_Float3, HLSLBaseType_Float4,
			HLSLBaseType_Float3x3, HLSLBaseType_Float4x4,
		...
		{ // float2x2
			...
			HLSLBaseType_Float2x2, HLSLBaseType_Unknown, HLSLBaseType_Unknown, HLSLBaseType_Unknown, 
			HLSLBaseType_Float2x2, HLSLBaseType_Float2x2,

			HLSLBaseType_Float2x2, HLSLBaseType_Float2x2, HLSLBaseType_Unknown, HLSLBaseType_Unknown,
        ...
        { // float3x3
			...
			HLSLBaseType_Float3x3, HLSLBaseType_Unknown, HLSLBaseType_Unknown, HLSLBaseType_Unknown, 
			HLSLBaseType_Float3x3, HLSLBaseType_Float3x3,
        ...
        { // half
			...
			HLSLBaseType_Half3x3, HLSLBaseType_Half4x4, 
        ...
        { // half3x3
			...
			HLSLBaseType_Half3x3, HLSLBaseType_Half3x3,
...
bool HLSLParser::ParseTopLevel(HLSLStatement*& statement)
{
    ...
    else if (AcceptType(true, type, typeName, &typeFlags))
    {
        ...
        else
        {
            ...
            // Handle semantics and register.
            while (Accept(':'))
            {
				if (declaration->semanticCount < countof(declaration->semantic) && 
					AcceptIdentifier(declaration->semantic[declaration->semanticCount]))
				{
					declaration->semanticCount++;

                }
				else if (declaration->registerName || !Expect(HLSLToken_Register) || !Expect('(') || !ExpectIdentifier(declaration->registerName) || !Expect(')'))
                {
                    return false;
                }
            }
...
bool HLSLParser::ParseDeclarationAssignment(HLSLDeclaration* declaration)
{
    if (Accept('='))
    {
		HLSLBaseType bt = declaration->type.baseType;

        // Handle array initialization syntax.
		if (declaration->type.array ||
			(m_tokenizer.GetToken() == '{' && bt != HLSLBaseType_Unknown && _baseTypeDescriptions[bt].numComponents * _baseTypeDescriptions[bt].height > 1))
		{
			int numValues = 0;
