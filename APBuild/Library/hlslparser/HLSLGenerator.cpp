...
void HLSLGenerator::OutputArguments(HLSLArgument* argument)
{
    ...
    while (argument != NULL)
    {
        ...
        OutputDeclaration(argument->type, argument->name, &semantic, semantic ? 1 : 0, /*registerName=*/NULL, argument->defaultValue);
        argument = argument->nextArgument;
        ++numArgs;
    }
}
    ...
    while (statement != NULL)
    {
        ...
        else if (statement->nodeType == HLSLNodeType_Struct)
        {
            ...
            while (field != NULL)
            {
                if (!field->hidden)
                {
                    ...
                    OutputDeclaration(field->type, field->name, &semantic, semantic ? 1 : 0);
                    m_writer.Write(";");
                    m_writer.EndLine();
                }
                field = field->nextField;
        ...
        else if (statement->nodeType == HLSLNodeType_Buffer)
        {
            ...
            while (field != NULL)
            {
                ...
                {
                    m_writer.BeginLine(indent + 1, field->fileName, field->line);
                    OutputDeclaration(field->type, field->name, /*semantic=*/NULL, 0, /*registerName*/field->registerName, field->assignment);
                    m_writer.Write(";");
                    m_writer.EndLine();
                }
...
void HLSLGenerator::OutputDeclaration(HLSLDeclaration* declaration)
{
    ...
    OutputDeclarationBody(declaration->type, declaration->name, declaration->semantic, declaration->semanticCount, declaration->registerName, declaration->assignment);
...
void HLSLGenerator::OutputDeclarationBody(const HLSLType& type, const char* name,
	const char** semantic/*=NULL*/, int semanticCount/*=0*/, const char* registerName/*=NULL*/, HLSLExpression * assignment/*=NULL*/)
{
    m_writer.Write("%s", name);

    if (type.array)
    {
		ASSERT(semanticCount == 0);
        m_writer.Write("[");
    ...
    if (semantic != NULL) 
    {
		if (semanticCount == 0) semanticCount = 1;
		for (int i = 0; i < semanticCount;i++)
			m_writer.Write(" : %s", semantic[i]);
    }
...
void HLSLGenerator::OutputDeclaration(const HLSLType& type, const char* name, const char** semantic/*=NULL*/, int semanticCount/*=0*/, const char* registerName/*=NULL*/, HLSLExpression * assignment/*=NULL*/)
{
    OutputDeclarationType(type);
	OutputDeclarationBody(type, name, semantic, semanticCount, registerName, assignment);
}
