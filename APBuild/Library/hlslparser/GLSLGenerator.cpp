...
void GLSLGenerator::OutputExpression(HLSLExpression* expression, const HLSLType* dstType)
{
    ...
    else if (expression->nodeType == HLSLNodeType_FunctionCall)
    {
		...
		else if (String_Equal(functionName, "any") || String_Equal(functionName, "all"))
		{
			...
			if (GetFunctionArguments(functionCall, argument, 1) != 1)
			{
				Error("any()/all() expects 1 argument");
				return;
			}

			const char* zeroExpression = "0.0";

			if (argument[0]->expressionType.baseType == HLSLBaseType_Bool)
				zeroExpression = "false";
			else if (argument[0]->expressionType.baseType == HLSLBaseType_Int)
				zeroExpression = "0";


			m_writer.Write("any(notEqual(");
			OutputExpression(argument[0]);
			m_writer.Write(", ");
			m_writer.Write(argument[0]->expressionType.typeName);
			m_writer.Write("(");
			m_writer.Write(zeroExpression);
			m_writer.Write(")))");
			handled = true;
		}
