...
namespace Apoc3D
{
	namespace Graphics
	{
		namespace D3D9RenderSystem
		{
			...
			bool D3D9VertexShader::TryGetParamIndex(const String& paramName, int& result)
			{
			...
			bool D3D9VertexShader::TryGetSamplerIndex(const String& paramName, int& result)
			{
				const ShaderConstant* sc = m_constantTable->tryGetConstant(paramName);
				if (sc && sc->IsSampler)
				{
			...
			void D3D9VertexShader::SetVector2(int reg, const Vector2& value)
			{
				m_buffer[0] = value.X;
				m_buffer[1] = value.Y;
			...
			void D3D9VertexShader::SetVector3(int reg, const Vector3& value)
			{
				m_buffer[0] = value.X;
				m_buffer[1] = value.Y;
				m_buffer[2] = value.Z;
			...
			void D3D9VertexShader::SetVector4(int reg, const Vector4& value) 
			{
				m_device->getDevice()->SetVertexShaderConstantF(reg, value, 1);
			}
			...
			void D3D9VertexShader::SetVector2(const String &paramName, const Vector2& value) 
			{
				const ShaderConstant& cons = m_constantTable->getConstant(paramName);
				if (!cons.RegisterCount)
				{
			...
			void D3D9VertexShader::SetVector3(const String &paramName, const Vector3& value) 
			{
				const ShaderConstant& cons = m_constantTable->getConstant(paramName);
				if (!cons.RegisterCount)
				{
			...
			void D3D9VertexShader::SetVector4(const String &paramName, const Vector4& value) 
			{
				const ShaderConstant& cons = m_constantTable->getConstant(paramName);
				if (!cons.RegisterCount)
				{
				...
				m_device->getDevice()->SetVertexShaderConstantF(cons.RegisterIndex, value, 1);
			}
